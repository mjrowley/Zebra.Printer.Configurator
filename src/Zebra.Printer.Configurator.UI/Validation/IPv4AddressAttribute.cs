...
/// <summary>Thin DataAnnotations wrapper so Configure.razor's EditForm can use Core's IPv4Validator as the single source of truth.</summary>
public sealed class IPv4AddressAttribute : ValidationAttribute
{
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        var outcome = IPv4Validator.Validate(value as string);
        if (outcome.IsValid)
        {
        ...
        // identifier rather than the specific property, so ValidationMessage For="...StaticIpAddress"
